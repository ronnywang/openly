...
while ($obj = json_decode(fgets($fp))) {
	...
	if (count($docUrls)) {
		...
		foreach ($docUrls as $idx => $docUrl) {
			...
			if (!file_exists($target) or filesize($target) < 100) {
				...
				system(sprintf("wget --timeout 10 -O %s %s", escapeshellarg("{$values->billNo}-{$idx}.doc"), escapeshellarg($docUrl)));
    ...
    if (!file_exists($target) or filesize($target) < 100) {
        ...
        $docUrl = str_replace('http://', 'https://', $docUrl);
        system(sprintf("wget --timeout 10 -O %s %s", escapeshellarg("{$values->billNo}.doc"), escapeshellarg($docUrl)));
